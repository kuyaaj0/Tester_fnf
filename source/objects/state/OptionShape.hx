...
class FloatRect extends FlxSpriteGroup
{
	...
	function changeData(isAdd:Bool)
	{
		...
		if (isAdd)
			data += Math.pow(0.1, follow.decimals);
		else
			data -= Math.pow(0.1, follow.decimals);
	...
	function rectUpdate()
	{
		...
		if (display._frame.frame.width < 1)
			display._frame.frame.width = 1;
	...
	function onHold()
	{
		...
		if (rect.x < bg.x)
			rect.x = bg.x;
		if (rect.x + rect.width > bg.x + bg.width)
			rect.x = bg.x + bg.width - rect.width;
...
class StringRect extends FlxSpriteGroup
{
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.mouse.overlaps(upRect))
			upRect.color = 0x53b7ff;
		else
			upRect.color = 0xffffff;

		if (FlxG.mouse.overlaps(downRect))
			downRect.color = 0x53b7ff;
		else
			downRect.color = 0xffffff;
	...
	function onClick()
	{
		...
		{
			follow.curOption++;
			if (follow.curOption >= follow.options.length)
				follow.curOption = 0;
		...
		{
			follow.curOption--;
			if (follow.curOption < 0)
				follow.curOption = follow.options.length - 1;
			follow.setValue(follow.options[follow.curOption]);
			disText.text = follow.options[follow.curOption];
			follow.change();
		}
