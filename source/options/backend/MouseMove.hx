...
class MouseMove extends FlxBasic
{
    ...
    public function new(tar:Float, min:Float, max:Float, onClick:Dynamic->Void = null, needUpdate:Bool = true) {
        super();
    ...
    override function update(elapsed:Float) {
        ...
        if (target < minLimit) target = FlxMath.lerp(minLimit, target, Math.exp(-elapsed * 30));
        if (target > maxLimit) target = FlxMath.lerp(maxLimit, target, Math.exp(-elapsed * 30));
